...
(function() {
    ...
    //starField//
    //---------//
    var starNumberModifier, starNumber, starSpeed, dirX, dirY, oldX, oldY, stars, mouseCoords;
      stars = [];
      starSpeed = 0.0025;
    ...
    function starField() {
        ...
        for (var x = 0; x < starNumber; x++) {
            ...
            //draw stars
            //ctx.save();<--don't do this anywhere until you know it's necessary--
    ...
    cat = new Player('cat', 'cat.gif', width - 546, height - 562, 546, 562);
    ...
    function animate() {
        ...
        //main update function--a few functions "draw" outside of render(); but
        //everything should clearRect via this function.
        render();
        stats.end();
        requestAnimationFrame(animate);
    }
    //defining function responsible for detection
    ...
    cat.weapon.fire = function(laserL, laserR) {
        ...
        //here are our lerp maths to calculate where the next node will be stroked c in l1c, l2c is for current
        cat.weapon.l1c.x = cat.weapon.l1s.x + (mouseCoords.x - cat.weapon.l1s.x) * cat.weapon.speed;
        cat.weapon.l1c.y = cat.weapon.l1s.y + (mouseCoords.y - cat.weapon.l1s.y) * cat.weapon.speed;
        cat.weapon.l2c.x = cat.weapon.l2s.x + (mouseCoords.x - cat.weapon.l2s.x) * cat.weapon.speed;
        cat.weapon.l2c.y = cat.weapon.l2s.y + (mouseCoords.y - cat.weapon.l2s.y) * cat.weapon.speed;
    ...
    function drawRats(imgsrc, ratBoxW, ratBoxH, xPos, yPos, degrees, opacity) {
        ...
        //save context before applying rotatio
        ctx.save();
        ...
        ctx.drawImage(imgsrc, 0, 0, ratBoxW, ratBoxH, -ratBoxW / 2, -ratBoxH / 2, ratBoxW, ratBoxH);
        ctx.rotate(-degrees * Math.PI / 180);
        ctx.translate(-xPos - ratBoxW / 2, -yPos - ratBoxH / 2);
        ctx.restore();
    }
    ...
    function resizeCanvas() {
        //respecify the canvas dimensions first.
        c.width = window.innerWidth;
        c.height = window.innerHeight;
        width = c.width;
        height = c.height;
